...
const WebDevelopment = () => {
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      ...
      const lessonList = lessonSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
                ...
                <div className="mt-6 space-y-3">
                  <div className="flex items-center text-white/70">
                    <span className="text-sm">
                      {/* {lesson.duration || '5 minutes'} • {lesson.level || 'Beginner'} */}
                    </span>
                  </div>

                  <div className="flex space-x-3">
                    <button
                      onClick={() => navigate(`/WebDevelopment/quiz/${lesson.id}`)}
                      className={`
